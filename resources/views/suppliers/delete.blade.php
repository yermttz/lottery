...
<div class="rssbanner text-uppercase">
    <a href="/lotteries"><strong>Proveedores</strong></a>
...
<div class="text-center m-auto" style="width: 300px;">
    ...
    <h3>¿Estás seguro de Eliminar el Proveedor {{$supplier->name}}?
        <form action="/suppliers/{{$supplier->id}}" method="post">
            @csrf
            {{ method_field('DELETE') }}

            <button type="submit" class="rssbanner-btn w-100 text-uppercase">Eliminar Proveedor</button>
        </form>
</div>
